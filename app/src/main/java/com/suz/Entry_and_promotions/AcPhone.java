...
import com.suz.Entry_and_promotions.Retrofit.PhoneList;
...
public class AcPhone extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_covers_t2);

        covers = findViewById(R.id.textView3);
        tables = (LinearLayout) findViewById(R.id.table);


        RetroClient.getApiService().getPhone()
                .subscribeOn(Schedulers.io())
                .doOnNext(Phone->getMusicDao().insertPhone(Phone.getData()))
                .onErrorReturn(new Function<Throwable, PhoneList>() {

                        @Override
                        public PhoneList apply(@NonNull Throwable throwable) throws Exception {
                            if (RetroClient.NETWORK_EXCEPTIONS.contains(throwable.getClass())) {

                                PhoneList response = new PhoneList();
                                response.setData((ArrayList<Phone>) getMusicDao().getPhone());
                                return response;
                            } else return null;
    ...
    private void GeneratingЕable(List<com.suz.database.Phone> contactList) {
